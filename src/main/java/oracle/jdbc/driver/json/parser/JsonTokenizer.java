...
final class JsonTokenizer implements Closeable {
  ...
  enum JsonToken {
    ...
    NUMBER(Event.VALUE_DECIMAL, true),
  ...
  private int peek() {
    try {
      ...
      throw OracleJsonExceptions.IO.create(factory(), ioe);
    }
  }
  ...
  private RuntimeException unexpectedChar(int ch) {
    ...
    return OracleJsonExceptions.TOKEN_UNEXPECTED_CHAR.create(factory(), chToString(ch), 
      location.getLineNumber(), location.getColumnNumber());
  }

  private RuntimeException expectedChar(int unexpected, char expected) {
    JsonLocationImpl location = getLastCharLocation();
    return OracleJsonExceptions.TOKEN_EXPECTED_CHAR.create(factory(), chToString(unexpected), 
      location.getLineNumber(), location.getColumnNumber(), chToString(expected));
  }

  private String chToString(int ch) {
    StringBuilder builder = new StringBuilder();
    if (ch >= 32 && ch <= 126) { // printable range
      builder.append("'");
